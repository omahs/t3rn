...
pub mod pallet {
    use super::{weights,  types::*};
    use frame_support::{
        pallet_prelude::*,
        traits::{Currency, ReservableCurrency},
    };
    use frame_system::{ensure_root,pallet_prelude::*};
    use sp_runtime::{Percent, Perbill,traits::{Saturating,Zero}};
    use sp_std::collections::btree_map::BTreeMap;
    use t3rn_primitives::{common::{Range, OrderedSet , RoundIndex,}, staking::{Bond,ExecutorSnapshot, CandidateBondLessRequest,CancelledScheduledRequest,ScheduledRequest, StakingAction, ExecutorStatus, CapacityStatus,StakerAdded,}, treasury::Treasury};
	...
	pub(crate) type TopStakes<T: Config> = StorageMap<
		_,
		Twox64Concat,
		T::AccountId,
		Stakes<T::AccountId, BalanceOf<T>>,
		OptionQuery,
	>;
	...
	pub(crate) type BottomStakes<T: Config> = StorageMap<
		_,
		Twox64Concat,
		T::AccountId,
		Stakes<T::AccountId, BalanceOf<T>>,
		...
		fn select_top_candidates(now: RoundIndex) -> (u32, u32, BalanceOf<T>) {
			...
			if executors.is_empty() {
				...
				for (account, snapshot) in <AtStake<T>>::iter_prefix(last_round) {
					...
					stake_count =
						stake_count.saturating_add(snapshot.stakes.len() as u32);
