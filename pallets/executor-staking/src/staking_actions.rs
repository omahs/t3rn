use crate::{pallet::{
	BalanceOf, CandidateInfo, Config, StakerInfo, Error, Event,
	Pallet, Total,StakeScheduledRequests,
},types::StakerMetadata};
// use crate::Delegator;
// use frame_support::ensure;
use frame_support::{ensure, traits::{Get, ReservableCurrency}, dispatch::DispatchResultWithPostInfo, RuntimeDebug};
// use frame_support::{dispatch::DispatchResultWithPostInfo, RuntimeDebug};
use codec::{Decode, Encode};
use scale_info::TypeInfo;
use sp_runtime::traits::Saturating;
use sp_std::{vec, vec::Vec};
use t3rn_primitives::{ treasury::Treasury,common::RoundIndex, staking::{Bond,  ScheduledRequest, StakingAction, CandidateBondLessRequest,CancelledScheduledRequest, ExecutorStatus, CapacityStatus,StakerAdded,}};
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn staker_schedule_revoke_all(
		staker: T::AccountId,
	) -> DispatchResultWithPostInfo {
		let mut state = <StakerInfo<T>>::get(&staker).ok_or(<Error<T>>::NoSuchStaker)?;
	...
	pub fn stake_request_revoke_exists(
		executor: &T::AccountId,
		staker: &T::AccountId,
	) -> bool {
		<StakeScheduledRequests<T>>::get(executor)
...
mod tests {
	...
	fn test_cancel_request_with_state_does_nothing_when_request_does_not_exist() {
		let mut state = StakerMetadata {
			...
			total: 100,
			less_total: 100,
			status: crate::StakerStatus::Active,
		};
