...
use t3rn_primitives::{common::{OrderedSet,RoundIndex},staking::{CapacityStatus,Bond, ExecutorStatus, CandidateBondLessRequest, StakerAdded, StakerStatus,},treasury::Treasury};
	...
	Stakes<AccountId, Balance>
{
	...
	pub fn bottom_capacity<T: Config>(&self) -> CapacityStatus {
		match &self.stakes {
			x if x.len() >= T::MaxBottomStakesPerCandidate::get() as usize => {
				CapacityStatus::Full
			}
			x if x.is_empty() => CapacityStatus::Empty,
			_ => CapacityStatus::Partial,
	...
	> CandidateMetadata<Balance>
{
		...
		BalanceOf<T>: Into<Balance>,
	{
		...
		self.highest_bottom_stake_amount =
			bottom_stakes.highest_stake_amount().into();
		...
		BalanceOf<T>: Into<Balance> + From<Balance>,
	{
		...
		if top_stakes.stakes.len() as u32 == max_top_stakes_per_candidate {
			...
			top_stakes.total = top_stakes
				.total
				.saturating_sub(new_bottom_stake.amount);
			if matches!(self.bottom_capacity, CapacityStatus::Full) {
				less_total_staked = Some(self.lowest_bottom_stake_amount);
			}
		...
		BalanceOf<T>: Into<Balance> + From<Balance>,
	{
		...
		let increase_stake_count = if bottom_stakes.stakes.len() as u32
			== T::MaxBottomStakesPerCandidate::get()
		{
		...
		BalanceOf<T>: Into<Balance> + From<Balance>,
	{
		...
		// remove top stake
		let mut top_stakes = <TopStakes<T>>::get(candidate)
			.expect("CandidateInfo exists => TopStakes exists");
		...
		BalanceOf<T>: Into<Balance>,
	{
		// remove bottom stake
		let mut bottom_stakes = <BottomStakes<T>>::get(candidate)
			.expect("CandidateInfo exists => BottomStakes exists");
		let mut actual_amount_option: Option<BalanceOf<T>> = None;
		...
		BalanceOf<T>: Into<Balance> + From<Balance>,
	{
		...
		let mut top_stakes =
			<TopStakes<T>>::get(candidate).ok_or(Error::<T>::NoSuchCandidate)?;
