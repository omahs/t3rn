...
build_docker_images() {
  ...
  if ! docker inspect parachain-collator:polkadot-v0.9.22-keyed > /dev/null; then
    DOCKER_BUILDKIT=1 docker build \
      -t parachain-collator:polkadot-v0.9.22-keyed \
      -f pchain.Dockerfile .
  fi
}
...
build_para_chain_specs() {
  ...
  docker run parachain-collator:polkadot-v0.9.22-keyed build-spec \
      --disable-default-bootnode \
  > ./specs/pchain.json
  # set parachain id(s)
  ...
  docker run \
      -v "$(pwd)/specs:/usr/local/etc" \
      parachain-collator:polkadot-v0.9.22-keyed \
      build-spec \
